    ...
    if (newIdx != rootIdx) {
      ...
      do {
        ...
        if (data < tree[currentIdx].data) {
          //왼쪽으로 내려갔을 떄 끝이면 left에 요소의 물리적 위치 인덱스를 추가(연결정보 설정)
          if (tree[currentIdx].left == -1) {
            tree[currentIdx].left = newIdx;
            addFlag = true;
          }
          // 왼쪽으로 내려갔을 떄 끝이 아니라면 왼쪽의 요소보다 더 내려감
          else {
        ...
        // 루트 요솟값보다 더 크면 오른쪽으로 내려감(동일한 값은 없음)
          ...
          //왼쪽으로 내려갔을 떄 끝이면 right에 요소의 물리적 위치 인덱스를 추가(연결정보 설정)
          if (tree[currentIdx].right == -1) {
            tree[currentIdx].right = newIdx;
            addFlag = true;
          }
          // 오른쪽으로 내려갔을 떄 끝이 아니라면 오른쪽의 요소보다 더 내려감
          else {
